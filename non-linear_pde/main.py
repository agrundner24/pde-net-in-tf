...
options = {'mesh_size': [250, 250],     # How large is the (regular) 2D-grid of function values for a fixed t.
           ...
           'batch_size': 10,            # We take a batch of sub-grids in space
           ...
           'iterations': 10,            # How often to repeat optimization in the warmup-step
           'max_order': 6,              # Max-order of our hidden PDE. Note: max_order < filter_size!
...
for i in range(options['iterations']):
    coefs_new, _, param_new, loss_new = a.optimize_weights(stage='WARMUP', iteration=i)
    if loss_new < loss:
        coefs = coefs_new
        loss = loss_new
        param = param_new
...
print('The parameter of f is %.8f' % param)
print('----------------------------------------------')  # End of warmup
