...
class _variantcoelinear2d(PDESolver):
    ...
    def __init__(self, spectral_size, max_dt=5e-3, variant_coe_magnitude=1):
        ...
        self.a[0,1] = lambda x:np.zeros(x.shape[:-1])      # Represents d/dy
        self.a[1,0] = lambda x:np.zeros(x.shape[:-1])+10   # Represents d/dx
        self.a[0,2] = lambda x:np.zeros(x.shape[:-1])      # Represents d²/dy²
        self.a[1,1] = lambda x:np.zeros(x.shape[:-1])      # Probably represents d²/d(x,y)
        self.a[2,0] = lambda x:np.zeros(x.shape[:-1])      # Represents d²/dx²
...
def generate(options):
    ...
    for i in range(batch_size):
        ...
        # Plotting one sample out of curiosity (works):
